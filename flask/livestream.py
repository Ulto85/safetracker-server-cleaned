...
def apiis(path):
    ...
    image = Image.open("documents/GoProjects/safetracker-server/flask/photos/a.png").convert("RGB")
...
def watch(link,auth_token,lvi):
    ...
    if len(lolz['predictions']) != 0:
        ...
        response = s3.Bucket(bucket).upload_file("documents/GoProjects/safetracker-server/flask/photos/a.png", str(uid) + "/livefeed/" + svid + ".png")
